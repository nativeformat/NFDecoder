...
void FactoryNormalisationImplementation::createDecoder(
  ...
  _wrapped_factory->createDecoder(
      path,
      mime_type,
      [create_decoder_callback, error_decoder_callback, samplerate, channels](std::shared_ptr<Decoder> decoder) {
        if (!decoder) {
        ...
        auto normalised_decoder = std::make_shared<DecoderNormalisationImplementation>(decoder, samplerate, channels);
